...
func TestBuildGLibc_AlreadyBuilt(t *testing.T) {
	...
	writeFakeTarballPlain(t, filepath.Join(srcDir, "glibc-2.41.tar.xz"))

	// Create src dir (already extracted)
	srcPath := filepath.Join(srcDir, "glibc-2.41")
	if err := os.MkdirAll(srcPath, 0755); err != nil {
		t.Fatal(err)
	}
...
func TestBootstrapAll_SkipToolchainFalseRunsBuildSteps(t *testing.T) {
	...
	for _, pkg := range []string{"binutils-2.42", "gcc-14.1.0", "glibc-2.41"} {
